...
CHANNEL_FACTS = ('JUUS 是远程文字聊天软件，不是一个房间、港区中的具体地点或人人待命的空间。'
    '频道成员只表示可以看到消息，不证明正在在线、同处一地或彼此就在身边。'
    '角色与用户的现实位置均未知；只有本轮明确说明或可见的已发生生活记录才能说明位置。'
    '谈聊天中的「这里」可以指消息或频道，不能据此描述面对面的动作、陪坐或递物。')
TERMINAL = (CHANNEL_FACTS + '港区是人物生活的故事背景。你收到消息后像平常聊天一样回应；只有明确接下委托时才进入工作状态。只发送自己实际会打出的文字；'
    '不写括号动作、星号动作、第三人称旁白，不替用户描写行动。'
    '可以自然谈兴趣、看法和打算；声称刚做过的具体事情须有当前可见生活记录支持，不能把用户写成参与者，也不能把模拟活动当作工具执行证据。'
...
def context(actor_id):
    ...
    if state['version']!='legacy-database' and not state['hasOverride'] and state['latest']:
        base=state['latest']
    ...
    with session_scope() as session:
        ...
        from backend.chat.sticker_catalog import labels
...
def worldbook():
    return json.loads((Path(__file__).resolve().parents[2]/'resources/world/juus.json').read_text(encoding='utf-8'))


def world_entries():
    book = worldbook()
    from backend.characters.character_identity import roster_actors,source_names
    with session_scope() as session:
    ...
    entries = [{'id':'port-terminal', 'keywords':['港区','JUUS'], 'text':'港区是人物生活的故事背景，不是 JUUS 聊天窗口所在的地点。JUUS 是远程文字通讯软件；群成员并不因此同处一地、实时在线或待命。人物平时有自己的生活。', 'origin':'product', 'source':None, **provenance}]
    entries.extend({**e,**provenance} for e in json.loads((Path(__file__).resolve().parents[2]/'resources/world/global.json').read_text(encoding='utf-8')))
