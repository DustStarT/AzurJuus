...
PACKAGED=Path(__file__).resolve().parents[2]/'resources/ui/wiki-assets.json'
MAX_MEDIA=3_000_000
...
async def media(label,still=False):
    ...
    if label=='标枪疑惑':
        ...
        return FileResponse(Path(__file__).resolve().parents[2]/f'resources/ui/javelin.{suffix}',media_type=f'image/{suffix}')
