...
import time
import httpx
from backend.characters.terminal_characters import TERMINAL, context, lore
...
def decode_json_body(content):
    """Accept a single JSON object with a small provider-added wrapper."""
    body=content.strip()
    ...
    try:return json.loads(body)
        ...
        start=body.find('{')
        if start<0 or start>100:raise
        value,end=json.JSONDecoder().raw_decode(body[start:])
        if not isinstance(value,dict) or body[start+end:].strip():raise
        return value
...
def validate(value, source_id, detailed=False, facts=None, max_chars=None):
    ...
    limit = max_chars if max_chars is not None else (12000 if detailed else 180)
    if len(text) > limit:
        raise ValueError(f'回复过长，请保留必要内容，合计不得超过{limit}字。')
...
class ExpressionService:
    ...
    async def complete(self, messages, settings):
        ...
        plain_json=bool(settings.get('_plainJsonRetry'))
        payload={'model':settings['llmModel'], 'messages':messages,
            'temperature':.2 if plain_json else .75,
            'max_tokens':max(256,min(8192 if settings.get('_mindStage') in {'understand','decide','reflect','profile'} else 4096,
                int(settings.get('_structuredOutputTokens',1800)))),
            'response_format':{'type':'json_object'}}
        from backend.tasks.reasoning_policy import apply as apply_reasoning
        apply_reasoning(payload,settings)
        if plain_json:
            # Some OpenAI-compatible JSON modes occasionally return only
            # whitespace. The retry still asks for JSON, but avoids that mode.
            payload.pop('response_format',None)
        async with httpx.AsyncClient(timeout=max(5,min(70,int(settings.get('_structuredTimeout',15))))) as client:
            ...
            return decode_json_body(content)
        ...
        try:
            async with (self.chat_gate if phase == 'chat' else self.task_gate):
                ...
                if 'segments' not in data:
                    ...
                    related_work=(resolve_task_reference(self.c.store,actor['id'],intent,run.get('history'),
                        force=bool(run.get('taskEvidenceHint')))
                        if phase=='chat' else None)
                    work_followup=phase=='chat' and (
                        run.get('routeKind')=='followup' or
                        bool(related_work and not related_work.get('ambiguous')) or
                        asks_about_work(intent) and not DEICTIC.search(intent))
                    ...
                    mind = self.c.cognition.context(actor['id'], intent, peers=peers,
                        ...
                        include_prior_task_history=phase!='chat' and asks_about_work(run.get('prompt',''))) if self.c.cognition else ''
                    runtime=getattr(self.c.cognition,'runtime',None)
                    decision=None
                    if runtime and runtime.active(actor['id']):
                        stage='cognition'
                        cognitive_input=json.dumps({'intent':intent,'facts':facts,'relatedWork':related_work,
                            'history':run.get('history',[])[-12:],
                        ...
                        # Keep diagnostic appraisals and tradeoffs out of speech
                        # generation. The selected action and checked fact bundle
                        # are enough to render a character-consistent answer.
                        mind=json.dumps({'intent':{k:v for k,v in decision['intent'].items()
                            if k in {'action','purpose','method','targetId'}}},ensure_ascii=False)
                    ...
                    # A verified work follow-up needs room for an opinion, its evidence,
                    # and the actual reading limit. The greeting limit of 180 chars
                    # caused valid book answers to fail after the task had succeeded.
                    max_chars = 12000 if detailed else 900 if work_followup else 180
                    policy = TERMINAL + '返回 JSON：{"segments":["短消息"]}。来源与投递编号由程序绑定，无需生成 sourceIds。'
                    policy += ('可以展开，优先完整回答问题；正文通常不超过2000字，避免逐字复述整个事实包。' if detailed else
                        '回答先前工作的追问时，有证据就说具体看法、依据与核验范围；没有证据就简短询问。通常一两条气泡、合计不超过500字，上限900字。' if work_followup else
                        '通常一两条气泡、合计30至120字，上限180字。简单确认可以更短。')
                    ...
                    if audience and audience.get('kind') == 'peer':
                        ...
                        policy += '这是有多位成员的远程群聊，成员名单只表示有阅读频道的资格，不证明此刻在线或在同一个地方。只扮演当前角色，不代替别人回答。看清历史中的发言者，接续具体话题，不重复问候或总结，不假定他人的经历是自己的。通常一句短答。'
                    ...
                    if phase=='chat':
                        ...
                        if run.get('routeKind')=='clarify':
                            policy += '用户的委托范围不明确，尚未接单。只问一个会影响执行范围的具体问题，不声称已经开始读取或处理文件。'
                        if run.get('routeKind')=='followup' and not related_work:
                            policy += '用户正在追问先前工作，但当前没有匹配的可核验材料。不要断言自己从未看过这本书；请简短询问书名或任务线索，再按证据回答。'
                    ...
                    messages.append({'role':'system','content':'最后确认当前场景：这是远程文字聊天。人设和历史中的面对面描写不是当前事实；不能邀请对方坐在身旁、提醒脚边物品或实际递送食物。没有本轮事实包或可见生活事件支持时，不要自称刚洗澡、做饭、敷面膜、出门或正待在某处；可以只回应消息，不必编造正在做什么。只回应本轮问题，不重复结尾的客套和限制。'})
                    if related_work and related_work.get('ambiguous') and phase == 'chat':
                        messages.append({'role':'system','content':'当前“这本书”等指代不清，可核验来源有多份。请询问用户具体是哪一本，不要自行猜测或说自己没读过：'
                            + json.dumps(related_work['candidateNames'],ensure_ascii=False)})
                    elif related_work and phase == 'chat':
                        messages.append({'role':'system','content':'本角色亲自参与的可核验工作经历；只回答相关追问，不把未读取部分说成已读：'
                            + json.dumps(related_work,ensure_ascii=False)})
                    elif (detailed or work_followup) and phase == 'chat' and run.get('routeKind')!='followup':
                        # Generic follow-ups still use the original room; natural
                        # references above may retrieve a verified result across rooms.
                    ...
                    def checked(value):
                        ...
                        try:
                            ...
                            return validate(prepared,source,detailed,facts,max_chars=max_chars)
                        except ValueError as exc:
                    ...
                    for attempt in range(2):
                        try:
                            ...
                            if attempt or decision:
                                raise
                            if '未返回结构化正文' in str(exc) or isinstance(exc,json.JSONDecodeError):
                                expression_settings={**expression_settings,'_plainJsonRetry':True}
