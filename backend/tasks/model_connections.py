...
from backend.credentials import protect
from backend.models import ModelConnection, WorkspaceSetting
...
async def probe(base_url: str, model: str, api_key: str) -> dict:
    ...
    payload={'model':model,'messages':messages,'temperature':0,'max_tokens':48,
        ...
        'tool_choice':'auto'}
    if urlsplit(base_url).hostname=='api.deepseek.com' and model.startswith('deepseek-'):
...
async def probe_reasoning(base_url: str, model: str, api_key: str) -> dict:
    """Explicit, small capability probes; never return model text or reasoning."""
    base_url,model=normalize(base_url,model)
    headers={'Content-Type':'application/json','Authorization':'Bearer '+(api_key or 'local-no-auth')}
    common={'model':model,'messages':[{'role':'user','content':'只返回 JSON：{"ok":true}'}],
        'max_tokens':512}
    variants={
    ...
    async with httpx.AsyncClient(timeout=20,follow_redirects=False) as client:
        try:
            baseline=await client.post(base_url+'/chat/completions',headers=headers,json=common)
            if baseline.status_code in {400,422}:
                # Some reasoning-only APIs require max_completion_tokens. Probe
                # extensions only after finding an accepted base envelope.
                common.pop('max_tokens')
                common['max_completion_tokens']=512
                baseline=await client.post(base_url+'/chat/completions',headers=headers,json=common)
                if baseline.status_code==200:token_parameter='max_completion_tokens'
            if baseline.status_code!=200:
                return {name:'unknown' for name in variants}
        except httpx.RequestError:
            return {name:'unknown' for name in variants}
        for name,params in variants.items():
            try:
                response=await client.post(base_url+'/chat/completions',headers=headers,json={**common,**params})
            except httpx.RequestError:
                results[name]='unknown'
                continue
            results[name]=('supported' if response.status_code==200 else
                'unsupported' if response.status_code in {400,422} else 'unknown')
    results['tokenLimitParameter']=token_parameter
    return results
