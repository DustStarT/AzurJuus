...
from backend.tasks.capabilities import CATALOG, LocalCapabilities, phase_actions
from backend.credentials import redact
from backend.tasks.hermes_bridge import HermesBridge
from backend.tasks.run_store import RunStore, TERMINAL
from backend.characters.personality import expression_rules
from backend.tasks.collaboration_dialogue import CollaborationDialogue
...
class RunCoordinator:
    ...
    def launch(self, run_id):
        try:
            ...
            if run.get('mode') != 'chat' and getattr(self, 'social_engine', None):
                for actor_id in {run['actorId'], *(a['actorId'] for a in run.get('assignments', []))}:
                    self.social_engine.preempt_actor(actor_id)
        except KeyError:
            pass
        ...
        if run_id not in self.tasks or self.tasks[run_id].done():
            ...
            def forget(finished):
                ...
                if not self.shutting_down and getattr(self, 'social_engine', None):
                    try:
                        run = self.store.get(run_id)
                        if run.get('mode') != 'chat':
                            asyncio.create_task(self.social_engine.resume_deferred(), name='resume-deferred-social')
                    except KeyError:
                        pass
            task.add_done_callback(forget)

    def busy_actor_ids(self):
        """Actors with an active task phase; paused runs do not occupy a speaker."""
        busy = set()
        for run in self.store.list(all_rows=True):
            if run.get('mode') == 'chat' or run.get('status') not in {'queued', 'running', 'waiting_approval'}:
                continue
            assignments = run.get('assignments') or []
            active = [item['actorId'] for item in assignments if item.get('status') == 'running']
            if active:
                busy.update(active)
            elif not assignments or run.get('status') == 'queued' or all(
                item.get('status') in {'completed', 'submitted'} for item in assignments
            ):
                busy.add(run['actorId'])
        return busy

    async def start(self):
        if getattr(self, 'moments', None):
            self.moments.initialize(restart=True)
    ...
    async def admit(self, payload, actors, settings, history, start_now=True):
        ...
        if payload.get('mode', 'task') != 'chat' and (not settings.get("authorizedWorkspaceRoot") or not root.is_dir()):
            raise ValueError("请先在设置中选择有效的授权工作区。")
        if not actors:
            raise ValueError("请先连接至少一名角色。")
        data = {"prompt": prompt, "conversationId": payload["conversationId"], "actors": actors, "actorId": actors[0]["id"], "workspace": str(root.resolve()) if settings.get("authorizedWorkspaceRoot") and root.is_dir() else '', "mode": payload.get("mode", "task"), "collaborative": bool(payload.get("collaborative")), "visionEnabled": bool(settings.get("visionEnabled")), "history": history[-40:], "grants": [], "headless": bool(payload.get("headless", False)), "taskEvidenceHint": bool(payload.get('taskEvidenceHint')), "routeKind":str(payload.get('routeKind') or '')}
        ...
        try:
            ...
            if not run["assignments"]:
                if run["collaborative"]:
                    ...
                    from backend.characters.terminal_settings import read
    ...
    async def execute_actor(self, run, actor, phase, prompt, settings):
        ...
        if phase != 'chat' and getattr(self, 'social_engine', None):
            self.social_engine.preempt_actor(actor['id'])
        ...
        async with (self.interaction_gate if discussion or phase == 'chat' else self.gate):
            ...
            from backend.characters.character_identity import material_context
            ...
            if phase == "chat" or discussion:
                policy = "保持角色口吻自然回答。当前为聊天处理，无工具授权；涉及本地操作时明确说明尚未执行，请用户提出明确委托或手动选择任务。"
            if discussion:
                policy = "你在参与正在进行的任务讨论。当前无工具，只能依据给出的证据交流，不要让用户切换任务模式。"
            from backend.characters.terminal_characters import CHANNEL_FACTS
            policy += CHANNEL_FACTS
