...
def backup_social_migration(engine, settings):
    if engine.dialect.name != 'sqlite' or engine.url.database == ':memory:':
        return
    with engine.connect() as connection:
        if not engine.dialect.has_table(connection, 'actors') or engine.dialect.has_table(connection, 'social_topics'):
            return
    import shutil
    folder = settings.workspace_state_path.parent / 'migration-backups' / (datetime.now().strftime('%Y%m%d-%H%M%S-%f') + '-social-v3')
...
def backup_moments_migration(engine, settings):
    """Keep old posts intact before adding publication and audience records."""
    if engine.dialect.name != 'sqlite' or engine.url.database == ':memory:':
        return
    with engine.connect() as connection:
        if not engine.dialect.has_table(connection, 'social_posts') or engine.dialect.has_table(connection, 'social_publications'):
            return
    folder = settings.workspace_state_path.parent / 'migration-backups' / (
        datetime.now().strftime('%Y%m%d-%H%M%S-%f') + '-moments-v5')
    folder.mkdir(parents=True, exist_ok=True)
    for label, source in [('business.db', Path(engine.url.database)),
                          ('runs.db', settings.workspace_state_path.parent / 'runs.db')]:
        if source.exists():
            with sqlite3.connect(source) as src, sqlite3.connect(folder / label) as dst:
                src.backup(dst)
    if settings.workspace_state_path.exists():
