...
class MindFrame(Record):
    ...
    alternativeInterpretation: str = Field(default='', max_length=300)
    valueTension: str = Field(default='', max_length=300)
...
class DecisionIntent(Record):
    action: Literal['speak','wait','end','execute','read','practice','rest','reflect','invite','stroll','meal','hobby','create_group','accept','decline','continue','finish','pause','contact']
...
class ProfileInterpretation(Record):
    ...
    attentionPriorities: list[str] = Field(default_factory=list, max_length=4)
    ambiguityStyle: str = Field(default='', max_length=300)
    valueTradeoffs: list[str] = Field(default_factory=list, max_length=4)
    methodFallbacks: list[str] = Field(default_factory=list, max_length=4)
...
class LifeSettings(Record):
    ...
    momentsEnabled: bool = True
    momentsActorDailyPosts: int = Field(default=1, ge=0, le=10)
    momentsDailyPosts: int = Field(default=3, ge=0, le=30)
    momentsActorDailyInteractions: int = Field(default=3, ge=0, le=30)
    momentsDailyInteractions: int = Field(default=8, ge=0, le=100)
