...
from backend.mind.mind_models import MindControl, LifeActivity, LifeRecord, PersonalGoal, MindProfile
from backend.mind.cognition_models import Experience, MindState, MindOutbox
from backend.mind.mind_contracts import LifeEvent


class LifeRuntime:
    ACTIONS={'read':'阅读已有资料','practice':'兴趣练习','rest':'休息','reflect':'独处整理',
        'invite':'共同交流','stroll':'散步','meal':'用餐','hobby':'兴趣活动'}
    ...
    def repair_contact_attribution(self):
        ...
        from backend.mind.mind_runtime import key
    ...
    def quiet(self, now=None):
        ...
        from backend.platform.local_clock import snapshot
    ...
    async def converse(self, activity, actor_id, decision):
        from backend.mind.mind_runtime import key, MindInterrupted
    ...
    async def contact(self, actor_id, decision):
        ...
        profile=self.r.profile(actor_id)
        revision=self.r.revision
        visible_room=self.r.c.social_engine.snapshot(cid,actor_id)
        audience={'kind':kind,'channel':visible_room['title'],
            'peers':[{'id':a['id'],'name':a['name']} for a in visible_room['members'] if a['id']!=actor_id]}
        intent=(('这是和频道同伴的自主群聊；用户只是可读成员，本轮没有向你提问。'
            '围绕自己的近况或想法与同伴交流，不请示、催促或等待用户回复。') if kind=='group' else
            '这是给用户的私聊；有相关内容才分享自己的近况，或简短关心对方，不催促回复。')
        value=await self.r.call(actor_id,'express',{'profile':profile['interpretation'],
            'intent':intent,'audience':audience,
            'facts':events,'sourceId':source},background=True,revision=revision)
        segments=validate(value,source)
        if kind=='group':
            from backend.social.social_engine import USER_WAIT
            if USER_WAIT.search('\n'.join(segments)):
                # An unsolicited group opening cannot assign the next turn to
                # a user who has not spoken. Skip this optional message.
                self.r.c.store.event(None,'mind.contact_skipped',
                    {'actorId':actor_id,'conversationId':cid,'reason':'waiting_for_user'})
                return
    ...
    async def tick(self):
        ...
        # User-requested research takes the next slot before optional life work.
        # Keep it single-filed; the two actor slots below are only for life work.
        if research.has_pending() and time.monotonic()-getattr(self,'last_research',-30)>=30:
            await self._tick_once()
            return
        await asyncio.gather(self._tick_once(),self._tick_once())
    ...
    async def _tick_once(self):
        ...
        async with self.r.background_gate:
            ...
            with session_scope() as s:
                ...
                attempts=(control.data.get('attempts',{}) if control else {})
                wake=(control.data.get('wake',{}) if control else {})
                now=time.time()
                candidates=[a.id for a in actors if self.r.mind.state(s,a.id).enabled and a.id not in busy
                    and (now-attempts.get(a.id,0)>120 or now<attempts.get(a.id,0))]
            # ready() may initialize actor state. Never invoke it from inside the
            # queue's SQLite write transaction, which would deadlock two slots.
            ready=[aid for aid in candidates if self.ready(aid,wake.get(aid,{}),now)]
            if not ready:return
            with session_scope() as s:
                ...
                available=[candidate for candidate in ready
                    if now-attempts.get(candidate,0)>120 or now<attempts.get(candidate,0)]
                if not available:return
                aid=min(available,key=lambda candidate:attempts.get(candidate,0))
            ...
            try:
                ...
                if activity:
                    ...
                    actions=['read','practice','rest','reflect','invite','stroll','meal','hobby','wait']
                    with session_scope() as s:
