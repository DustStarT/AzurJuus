...
from backend.mind.cognition_models import MindState, Experience, MindCursor, MindReceipt, MindOutbox
...
class Cognition:
        ...
        if kind == 'mind.observation':
            ...
            for aid in participants:
                ...
                yield aid, 'outcome', f"任务状态：{payload['status']}。{summary}", {
                    ...
                    'readSources': read_sources(self.store,payload,aid) if payload['status']=='completed' else [],
    ...
    def relationships(self, actor_id):
        from backend.characters.terminal_characters import world_entries
        ...
        with session_scope() as session:
            ...
            for row in baseline:
                ...
                from backend.characters.character_identity import default_relationship
