...
from backend.mind.cognition_models import Experience, MindState, MindOutbox
from backend.mind.mind_models import MindProfile, MindDecision, PersonalGoal, MindControl, MindCall, LifeActivity
from backend.mind.mind_contracts import MindFrame, DecisionIntent, StateDelta, ProfileInterpretation, LifeSettings
...
class MindRuntime:
    def __init__(self, mind, coordinator):
        ...
        # Two different actors may consider optional life activities at once.
        # Actor-local locks in think() still serialize one actor's state.
        self.background_gate = asyncio.Semaphore(2)
        self.active_calls = 0
        self.peak_calls = 0
        self.revision = 0
        self.generate = None
        self.background_context = ContextVar('mind-background', default=None)
        from backend.mind.life_runtime import LifeRuntime
    ...
    def control(self, patch=None):
        with session_scope() as s:
            ...
            if patch is not None:
                ...
                self.revision += 1
            # Additive settings remain valid for databases created before a
            # feature was introduced; persisted user choices win over defaults.
            cfg = LifeSettings(**row.data['settings']).model_dump()
            if cfg != row.data['settings']:
                row.data={**row.data,'settings':cfg}
            ...
            all_stages={}
            for call in calls:
                stage=all_stages.setdefault(call.kind,{'kind':call.kind,'calls':0,'failures':0,'tokens':0})
                stage['calls']+=1
            ...
            for call in background_calls:
                ...
                stage['failures']+=call.data.get('status')=='failed'
                stage['tokens']+=(call.data.get('usage') or {}).get('total_tokens',0) or 0
            from backend.platform.local_clock import snapshot as current_time
            queue=s.get(MindControl,'queue')
            recent_decisions=s.scalars(select(MindDecision).where(MindDecision.at>time.time()-3600)).all()
            return {'enabled':self.enabled, 'settings':cfg,
                'time':current_time(),'scheduled':(queue.data.get('wake',{}) if queue else {}),
                'activeCalls':self.active_calls,'peakConcurrentCalls':self.peak_calls,
                'decisionQueueWaitSeconds':sum(r.data.get('waitSeconds',0) for r in recent_decisions),
                'emptyResponsesLastHour':sum(bool(r.data.get('emptyResponse')) for r in calls),
                'expiredDecisionsLastHour':sum(r.status=='interrupted' for r in recent_decisions),
                'staleGroupDecisionsLastHour':sum(r.data.get('publication')=='stale' for r in recent_decisions),
                ...
                'stagesLastHour':sorted(all_stages.values(),key=lambda stage:stage['kind']),
    ...
    async def call(self, actor_id, kind, payload, schema=None, background=False, revision=None, messages=None, generator=None, settings=None, validator=None):
        for attempt in range(3):
            try:
                ...
                empty_body='未返回结构化正文' in str(exc)
                malformed_json=isinstance(exc,json.JSONDecodeError)
                # A whitespace-only response can be produced by a provider's
                # JSON mode even with finish_reason=stop. Switch formats on the
                # next attempt; allow one more bounded attempt only for this case.
                if attempt >= (2 if empty_body or malformed_json else 1):raise
                ...
                if empty_body or malformed_json:
                    settings={**(settings or self.c.settings_loader()),'_plainJsonRetry':True}
    ...
    async def _call_once(self, actor_id, kind, payload, schema=None, background=False, revision=None, messages=None, generator=None, settings=None, validator=None):
        ...
        rules = {
            'profile':'从人物背景编译可纠正的作者解释。身份与核心价值稳定；保留例外和不确定性。提炼人物优先注意什么、遇到歧义如何核实、价值冲突时如何取舍及惯用方法失败后的替代办法；不能把人物背景当作真实工具能力。原作中对指挥官说的台词只反映那段关系，不意味着人物与同伴聊天也始终围着指挥官转。',
            'understand':'理解当前可见事件。区分发生的事实、角色猜测和未知；有真实歧义时写出一种有依据的其他解释和相关价值冲突，普通问候留空。依据人物 attentionPriorities 和 ambiguityStyle 选择关注点，不机械复述背景。消息的发言者只由 speakerId 判定；记忆中 ownSpeech 为 false 表示听到别人说话，不是本人自述，话语也不证明其内容已经发生。commander 是用户，用户称呼了某角色不等于用户就是该角色，也不改变当前 actorId。情绪须有对象与原因。不输出长篇内心独白。',
            'decide':'根据认知快照选择一个允许的行动。依据人物 values、valueTradeoffs、methods 和 methodFallbacks 比较相关选项；只有确有取舍时用 tradeoff 简短说明理由及放弃的办法。保留人物分歧与方法差异，承诺后的工作必须可靠。只有相关来源才能支持目标。不要代替他人同意，不创造工具能力。goal 只在需要新长期目标时提出，否则为 null。goalUpdate 仅在已有目标的 evidence 支持下一步或状态改变时提出；使用该目标已有 evidence 中的编号，不能把模拟活动当现实能力认证。',
        ...
        if kind=='decide' and payload.get('mode')=='social':
            instruction+='\n群聊参与判断优先于表达：只有尚未回答的问题、具体的新信息、真实的不同意见或新的相关问题才值得接话。最近几句已经表示赞同、总结、待命或等别人说话时，选择 wait/end；改写别人说过的话、再感谢一次、再邀请大家发言不算新内容。不要为了轮流说话而选择 speak。用户只是群成员之一；最近发言者是同伴时，优先回应其具体观点或自己的相关目标，不把同伴对话转成等待用户回来、回复或批准。'
        if schema:
        ...
        task = None
        error_kind = ''
        complex_call = kind=='decide' and bool(payload.get('frame',{}).get('alternativeInterpretation') or
            payload.get('frame',{}).get('valueTension') or payload.get('mode')=='task' and payload.get('frame',{}).get('unknowns'))
        mind_stage = kind in {'profile','understand','decide','reflect'}
        output_tokens = cfg.get('_structuredOutputTokens') or (8192 if complex_call else 4096 if mind_stage else 2400)
        timeout = 70 if mind_stage else 40
        self.active_calls += 1
        ...
        try:
            ...
            task = asyncio.create_task(generator(messages, {**cfg,'_mindCallId':call_id,'_mindStage':kind,
                '_mindComplex':complex_call,'_structuredOutputTokens':output_tokens,
                '_structuredTimeout':cfg.get('_structuredTimeout',timeout)}))
            while not task.done():
            ...
            with session_scope() as s:
                ...
                if row:
                    row.data={**row.data,'status':status,'latencySeconds':time.monotonic()-started,
                        'errorKind':error_kind,'emptyResponse':bool(error_kind=='ValueError' and
                            task and task.done() and isinstance(task.exception(),ValueError) and
                            '未返回结构化正文' in str(task.exception()))}

    def profile(self, actor_id):
        from backend.characters.terminal_characters import inspect
        from backend.characters.character_behavior import behavior_context
    ...
    def snapshot(self, actor_id, prompt, source, conversation_id='', public=False, task_run_id='', include_task_memory=True,
        ...
        with session_scope() as s:
            ...
            if related_run_id:
                selected=s.scalars(select(Experience).where(Experience.actor_id==actor_id,
                    Experience.run_id==related_run_id,Experience.kind=='outcome',
                    Experience.forgotten.is_(False))).all()
                present={r['id'] for r in rows}
                rows.extend({'id':r.id,'text':r.text,'kind':r.kind,'at':r.at,'runId':r.run_id,'data':r.data}
                    for r in selected if r.id not in present and r.data.get('verified'))
        # An actor may remember an event without disclosing it in another room.
        # Unknown legacy channel attribution is never interpreted as public speech.
        if conversation_id:
            rows=[r for r in rows if r['data'].get('shareable') or
                r['data'].get('conversationId')==conversation_id or
                related_run_id and r['kind']=='outcome' and r['runId']==related_run_id and r['data'].get('verified') or
                not public and not r['data'].get('conversationId') and
                    r['kind'] not in {'speech','request','outcome'}]
        elif public:
            rows=[r for r in rows if r['data'].get('shareable') or
                related_run_id and r['kind']=='outcome' and r['runId']==related_run_id and r['data'].get('verified')]
        if task_run_id:
            ...
            from backend.chat.conversation_scope import asks_about_work, task_experience
            modes={}
            rows=[r for r in rows if not task_experience(r,self.mind.store,modes)
                or r['kind']=='outcome' and r['data'].get('verified') and
                    (r['runId']==related_run_id or not related_run_id and asks_about_work(prompt))]
        elif not include_task_memory:
            from backend.chat.conversation_scope import task_experience
            modes={}
            rows=[r for r in rows if not task_experience(r,self.mind.store,modes)]
        goal_text=' '.join(g['title']+' '+g.get('nextStep','') for g in goals)
        rows.sort(key=lambda r:(bool(related_run_id and r['runId']==related_run_id and r['kind']=='outcome'),
            bool(r['data'].get('userCorrection')),
        ...
        if os.getenv('AZURJUUS_MIND_DECISION_STYLE','character')=='generic':
            # Evaluation ablation: omit newly compiled decision preferences,
            # while keeping identity and factual provenance available.
            for field in ('attentionPriorities','ambiguityStyle','valueTradeoffs','methodFallbacks'):
                profile_summary.pop(field,None)
        ...
        return {'actorId':actor_id,'version':state['version'],'profileVersion':profile['version'],'time':current_time(),
            'channelContext':{'medium':'remote_text','coLocated':False,'physicalLocation':'unknown',
            ...
            'conversationId':conversation_id,'sessionKey':key(actor_id,conversation_id or 'private-mind')}

    async def think(self, actor_id, source, prompt, *, mode='chat', conversation_id='', public=False,
                    background=False, actions=None, task_run_id='', include_task_memory=None,related_run_id=''):
        for attempt in range(3):
            try:
                return await self._think_once(actor_id,source,prompt,mode=mode,conversation_id=conversation_id,
                    ...
                    include_task_memory=include_task_memory,related_run_id=related_run_id)
        ...
        async with self.locks[actor_id]:
            ...
            try:
                ...
                def validate_decision(value):
                    ...
                    if mode in {'social','moments','life-share'} and (value.goal or value.goalUpdate):
                        raise ValueError('社交判断不能直接改变个人目标。')
                    if not set(value.sourceIds)<=set(snap['allowedSources']):
                    ...
                    if value.goalUpdate:
                        ...
                        if (value.goalUpdate.status!='active' and value.goalId==value.goalUpdate.goalId
                                and value.action in {'read','practice','rest','reflect','invite','stroll','meal','hobby','continue'}):
                ...
                with session_scope() as s:
                    ...
                    row.data={**row.data,'id':did,'frame':frame.model_dump(),'intent':decision.model_dump(),
                        ...
                        'publication':'considered' if mode=='social' else 'not_applicable'}
                    if decision.goal:
    ...
    def mark_social_publication(self, decision_id, action):
        with session_scope() as s:
            row=s.get(MindDecision,decision_id)
            if row and row.status=='committed' and row.data.get('mode')=='social':
                row.data={**row.data,'publication':action}
