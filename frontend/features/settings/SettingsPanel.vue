...
import type { Workspace, Settings, Agent } from "../../shared/types";
import Icon from "../../shared/Icon.vue";
import Avatar from "../../shared/Avatar.vue";
import TerminalConfiguration from '../characters/TerminalConfiguration.vue';
import LifeSettings from '../life/LifeSettings.vue';
...
type ModelCheck = {available:boolean;toolCalling:string;message:string;latencyMs?:number;httpStatus?:number;
  reasoning?:{structuredOutput:string;reasoningEffort:string;thinkingMode:string;tokenLimitParameter?:string}};
...
async function checkModel(checkReasoning = false) {
  checkingModel.value=true; modelCheck.value=null; error.value='';
  try { modelCheck.value=await api<ModelCheck>('/api/model/check',{
    baseUrl:settings.value.llmBaseUrl,model:settings.value.llmModel,apiKey:settings.value.llmApiKey || '',checkReasoning});
      ...
      <div class="settings-layout">
        ...
        <div class="settings-content">
          ...
          <template v-if="tab === 'connection'"
            ...
            ><label
              ...
              ><input
                ...
                " /></label
            >
            <div class="inline-actions"><button class="soft-button" :disabled="checkingModel || changingModel || !settings.llmBaseUrl.trim() || !settings.llmModel.trim()" @click="checkModel(false)">{{checkingModel ? '检查中…' : '检查模型可用性'}}</button><button class="text-button" :disabled="checkingModel || changingModel || !settings.llmBaseUrl.trim() || !settings.llmModel.trim()" @click="checkModel(true)">检查推理能力</button></div>
            ...
            <p v-if="modelCheck?.reasoning" class="field-hint">结构化输出：{{modelCheck.reasoning.structuredOutput}} · 推理强度：{{modelCheck.reasoning.reasoningEffort}} · 思考模式：{{modelCheck.reasoning.thinkingMode}}。不支持的参数不会发送；检查结果不包含模型推理文字。</p>
          ...
          <template v-if="tab === 'appearance'"
            ...
            <h3>让港区慢下来</h3>
            <p class="field-hint">聊天背景可在私聊右上角“调整背景”中设置位置与缩放，每个角色单独保存。</p>
