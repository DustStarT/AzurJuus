...
type Edge={from:string;to:string;summary:string;background:{text:string;source?:string;quote?:string;origin?:string}[];sharedSources:string[];userDefined:string;defaultRelationship:{familiarity:string}};
const nodes=ref<Node[]>([]),edges=ref<Edge[]>([]),selected=ref(''),error=ref(''),source=ref('');
const emit=defineEmits<{select:[string]}>();
async function load(){try{const data=await api<{nodes:Node[];edges:Edge[]}>('/api/relationships/graph');nodes.value=data.nodes;edges.value=data.edges;}catch(e){error.value=(e as Error).message;}}
...
const statuses:Record<string,string>={pending:'等待后台研究',completed:'研究完成',failed:'研究失败，可重试'};
...
<template><section aria-label="人物关系网络"><h3>关系网络</h3><p>只显示同阵营熟悉关系、有出处的互动、用户设定或实际合作；不同阵营的默认认识不画线。</p><button class="text-button" @click="load">刷新关系图</button><p v-if="error" role="alert">{{error}}</p>
