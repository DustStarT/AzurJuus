...
async def main():
    ...
    home = ROOT / '.azurjuus' / 'cloud-api-acceptance' / uuid4().hex
    work = home / 'work'
    ...
    os.environ.update(AZURJUUS_DATABASE_URL='sqlite+pysqlite:///' + (home / 'app.db').as_posix(),
        ...
        AZURJUUS_REDIS_URL='', AZURJUUS_CHROMA_URL='', AZURJUUS_SOCIAL_ENABLED='0', AZURJUUS_EXECUTION_BACKEND='hermes')
    from server import create_server
    server = create_server(host='127.0.0.1', port=0)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    url = 'http://127.0.0.1:' + str(server.server_address[1])
    report = {'model': model, 'status': 'failed', 'checks': [], 'workspace': str(work)}
    try:
        async with httpx.AsyncClient(base_url=url, timeout=30) as client:
            ...
            payload = {'conversationId': cid, 'mode': 'task', 'requestId': uuid4().hex,
            ...
            messages = final['snapshot']['messages'][cid]
            assert len([m for m in messages if m['id'] == rid + '-user']) == 1
            assert len([m for m in messages if m['id'] == rid + '-result']) == 1
        ...
        server.server_close()
        (ROOT / 'validation/cloud-api-report.json').write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding='utf-8')
