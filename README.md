...
当前角色体验仍在迭代：十二名基础角色卡是简要作者解释，原作剧情并未被完整导入。设置中的关系图可对**当前名单**一键重新检索 Wiki 的原作聊天、JUUs 动态和剧情；引文核验通过后，关系与人物背景理解才进入运行上下文。研究结果会记录来源、未完成批次和进度，不能视为角色在本应用真实经历。表情贴纸按需从 Wiki 的游戏聊天及官方分区下载，可跨角色使用。群聊现在优先接最近发言，允许沉默，不再为了维持轮数强制重答最初的问题；自然度仍需真实模型和人工对照验收。
...
更完善的角色扮演的关系建立还在实验中：
群聊默认使用自主社交调度，支持按当前对话选择参与者、邀请和建群；设置 `AZURJUUS_SOCIAL_ENGINE_ENABLED=0` 可回退旧路径。用户也可自行新建群聊并从当前名单选人，或在群设置中加入成员；社交群资格不授予工具权限。世界观设置只显示全局背景，人物材料与关系分别查看。资料研究会从角色页相关活动进入剧情文字版，按 Wiki 名称对照表识别和谐名，引用只在对应场景附近校验；这仍是可纠正的模型解释。十二人的终端基础卡已整理；已有用户修改保留。当前验证和限制见 [人物启用记录](docs/SOCIAL_ROSTER_20260920.md)、[自主社交进展](docs/SOCIAL_ACTIVITY_20260920.md)及[关系资料记录](docs/RELATIONSHIP_RESEARCH_20260921.md)。离线与界面回归通过，最新真实社交复验遭遇模型 HTTP 402；自然度与整轮桌面验收仍需持续评价。
