...
AzurJuus 是一个在 Windows 本机运行的角色化 Agent 桌面应用。界面采用《碧蓝航线》JUUS 通讯与朋友圈的形式，提供角色私聊、群聊、动态和工作抽屉；切换到「任务」或「协作」模式后，角色会实际读写文件、执行命令、读取 PDF/DOCX/XLSX、操作浏览器，或整理 Windows 文件资源管理器。
...
- **每个角色不是独立训练的模型。** 她们可以是同一个模型服务，靠角色卡、可见信息、有方向的关系、方法偏好和对话对象区分。项目没有训练或微调任何权重。
...
- **这是一个个人练手项目。** 工程机制有实现和回归测试，核心链路做过隔离验收，但没有长期稳定性验证。
...
界面左侧是会话列表与角色立绘，右侧是当前会话，底部切换闲聊 / 任务 / 协作。工作抽屉里的分工、工具调用、审批请求和交付文件都来自持久化事件，不是前端模拟的进度；可以随时暂停、继续、停止，或补充新要求。

界面上的几个处理：流式消息与最终消息共用同一个 DOM 节点，分段呈现由 `speechQueue.ts` 按会话串行调度，不改数据库正文；默认只渲染最近 100 条消息，更早的按批加载；输入框保护 `composition` 状态，中文输入法选字时按 Enter 不会误发送；历史加载和重连不重播动画。

## 交付与验收

任务是否完成由调度层判定，不是由模型的回复决定。成员回一句「已经整理好了」不会被当成完成。
...
也可以双击 `launch_azurjuus.bat` 或 `launch_azurjuus.vbs`。
